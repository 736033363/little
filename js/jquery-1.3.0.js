...
jQuery.fn = jQuery.prototype = {
    ...
    not: function( selector ) {
        ...
        return this.filter(function() { // 用filter过滤
    ...
    // 基本用于获取元素的value
    // 对于select，没有value属性的，获取其innerText
    // 可读可写
    // $("input").val() 只会获取第一个元素的值
    val: function( value ) {
        // 读
        if ( value === undefined ) {            
            var elem = this[0];
        ...
        return this.each(function(){
            if ( this.nodeType != 1 )
                return;
            // 处理type为radio，checkbox，返回true or false
    ...
    // 把制定索引的dom对象从jQuery中取出，还是一个jQuery对象
    eq: function( i ) {
        return this.slice( i, +i + 1 );
    },
    // 对当前jQuery中的DOM进行slice操作
    slice: function() {
...
// 把jQuery.fn的能力给到jQuery.fn.init.prototype
